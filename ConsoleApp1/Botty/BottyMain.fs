...
module BottyMain =
        ...
        commands.RegisterCommands<BottyComms>()
        ...
        let cmde_cbl (e: CommandErrorEventArgs) =
            ...
            embed.Footer.Text <- "no command 4 u"
        ...
        let timer_cbl (_: obj) =
            ...
            discord.UpdateStatusAsync(new DiscordGame(sprintf "Alive for: %s" ((DateTime.UtcNow - proc.StartTime.ToUniversalTime()).ToString()))) |> Async.AwaitTask |> Async.RunSynchronously
